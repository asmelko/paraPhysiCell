...
void SVG_plot(std::string filename, environment& e, PhysiCell_Settings& settings, double z_slice, double time,
			  ...
			  cell_counts_funct_t cell_counts_function)
{
					...
					for (index_t x = 0; x < e.m.mesh.grid_shape[0]; x++)
					{
						...
						{ // this is to make sure the substrate is sampled in the voxel visualized (so basically the
						  // slice)
							...
							std::string output =
								substrate_coloring_function(concentration, max_conc, min_conc, settings);
	...
	*/
	...
	for (int i = 0; i < total_cell_count; i++)
	{
		...
		{
			os << "   <g id=\"cell" << pC->id << "\" " << "type=\"" << pC->type_name << "\" "; // new April 2022
	...
	if (settings.enable_substrate_plot == true && substrate_coloring_function != NULL)
	{
		...
		{ // creating 10 rectangoles for the bar, each one with a different shade of color.
			...
			double concentration_sample =
				min_conc + (conc_interval * (9 - i)); // the color depends on the concentration, starting from the min
													  // concentration to the max (which was sampled before)

			std::string output = substrate_coloring_function(concentration_sample, max_conc, min_conc, settings);

			double upper_left_y = sub_rect_height * i; // here I set the position of each rectangole

			Write_SVG_rect(os, upper_left_x, top_margin + upper_left_y, 25.0, sub_rect_height, 0.002 * plot_height,
						   "none", output); // drawing each piece of the barplot
			...
			{ // of course I am not printing each value of the barplot, otherwise is too crowded, so just one each 2
				...
				Write_SVG_rect(os, upper_left_x + 25,
							   top_margin + upper_left_y + sub_rect_height - (0.001 * plot_height), 3,
							   0.002 * plot_height, 0, "rgb(0,0,0)", "rgb(0,0,0)");
				Write_SVG_text(
					os, szString, upper_left_x + 28, top_margin + upper_left_y + sub_rect_height, font_size,
					PhysiCell_SVG_options.font_color.c_str(),
					PhysiCell_SVG_options.font.c_str()); // misterious values set with a trial and error approach due to
														 // OCD. But now the legend is coherent at pixel level
			}
		}

		sprintf(szString, "%.2g", max_conc);

		Write_SVG_rect(os, upper_left_x + 25, top_margin - (0.001 * plot_height), 3, 0.002 * plot_height, 0,
					   "rgb(0,0,0)", "rgb(0,0,0)");
		Write_SVG_text(os, szString, upper_left_x + 28, top_margin, font_size, PhysiCell_SVG_options.font_color.c_str(),
					   PhysiCell_SVG_options.font.c_str()); // misterious values set with a trial and error approach due
															// to OCD. But now the legend is coherent at pixel level

		delete[] szString;

		// add a label to the right of the colorbar defined by above Write_SVG_rect calls
		Write_SVG_text(os, settings.substrate_to_monitor.c_str(), upper_left_x + 35, top_margin + plot_height / 2,
					   font_size, PhysiCell_SVG_options.font_color.c_str(), PhysiCell_SVG_options.font.c_str(), 90.0);
	}

	Write_SVG_rect(os, 25.0 + plot_width, top_margin, 25.0, plot_height - 25, 0.002 * plot_height, "black",
...
void create_plot_legend(std::string filename, cell_coloring_funct_t cell_coloring_function, environment& e)
{
	...
	{
		cell_definition* cell_def = e.cell_definitions[k].get();
		standard_agent_legend(os, cell_def, cursor_x, cursor_y, cell_coloring_function, temp_cell_radius, e);
		// cell_def->functions.plot_agent_legend(os, cell_def, cursor_x, cursor_y, cell_coloring_function,
		// 									  temp_cell_radius);
										...
										PhysiCell_Settings& settings)
{
	...
	{
		setup_svg_substrate_colormap(colormap, settings);
...
void standard_agent_SVG(std::ofstream& os, cell* pC, double z_slice, cell_coloring_funct_t cell_coloring_function,
						double X_lower, double Y_lower)
{
	...
	Write_SVG_circle(os, (pC->position())[0] - X_lower, (pC->position())[1] - Y_lower, plot_radius, 0.5, Colors[1],
					 Colors[0]);
	// plot the nucleus if it, too intersects z = 0;
	if (fabs(z) < rn && PhysiCell_SVG_options.plot_nuclei == true)
	{
		plot_radius = sqrt(rn * rn - z * z);
		Write_SVG_circle(os, (pC->position())[0] - X_lower, (pC->position())[1] - Y_lower, plot_radius, 0.5, Colors[3],
						 Colors[2]);
	}
}
						   ...
						   cell_coloring_funct_t cell_coloring_function, double temp_cell_radius, environment& e)
{
	...
	cell* C = e.get_container().create_cell(*cell_definition);
...
{
	std::string map_name = settings.svg_substrate_colormap;
	...
	{
		colormap.assign({ "rgb(255,255,204)", "rgb(255,250,193)", "rgb(255,246,181)", "rgb(255,241,170)",
						  "rgb(255,236,159)", "rgb(255,231,148)", "rgb(254,226,137)", "rgb(254,221,126)",
						  "rgb(254,214,115)", "rgb(254,204,104)", "rgb(254,194,94)",  "rgb(254,184,83)",
						  "rgb(254,174,74)",  "rgb(254,165,70)",  "rgb(253,155,66)",  "rgb(253,146,62)",
						  "rgb(253,133,58)",  "rgb(253,117,53)",  "rgb(252,100,48)",  "rgb(252,84,44)",
						  "rgb(248,70,40)",	  "rgb(242,56,36)",	  "rgb(235,43,33)",	  "rgb(229,29,29)",
						  "rgb(220,21,30)",	  "rgb(210,14,33)",	  "rgb(200,8,35)",	  "rgb(190,1,38)",
						  "rgb(175,0,38)",	  "rgb(159,0,38)",	  "rgb(144,0,38)",	  "rgb(128,0,38)" });
	}
	else if (map_name == "viridis")
	{
		colormap.assign({ "rgb(68,1,84)",	 "rgb(71,13,96)",	"rgb(72,24,106)",  "rgb(72,35,116)",  "rgb(71,46,124)",
						  "rgb(69,56,130)",	 "rgb(66,65,134)",	"rgb(62,74,137)",  "rgb(58,84,140)",  "rgb(54,93,141)",
						  "rgb(50,101,142)", "rgb(46,109,142)", "rgb(43,117,142)", "rgb(40,125,142)", "rgb(37,132,142)",
						  "rgb(34,140,141)", "rgb(31,148,140)", "rgb(30,156,137)", "rgb(32,163,134)", "rgb(37,171,130)",
						  "rgb(46,179,124)", "rgb(58,186,118)", "rgb(72,193,110)", "rgb(88,199,101)", "rgb(108,205,90)",
						  "rgb(127,211,78)", "rgb(147,215,65)", "rgb(168,219,52)", "rgb(192,223,37)", "rgb(213,226,26)",
						  "rgb(234,229,26)", "rgb(253,231,37)" });
	}
	...
	{
		colormap.assign(
			{ "rgb(0,0,131)",	  "rgb(0,0,135)",	  "rgb(0,0,139)",	  "rgb(0,0,143)",	  "rgb(0,0,147)",
			  "rgb(0,0,151)",	  "rgb(0,0,155)",	  "rgb(0,0,159)",	  "rgb(0,0,163)",	  "rgb(0,0,167)",
			  "rgb(0,0,171)",	  "rgb(0,0,175)",	  "rgb(0,0,179)",	  "rgb(0,0,183)",	  "rgb(0,0,187)",
			  "rgb(0,0,191)",	  "rgb(0,0,195)",	  "rgb(0,0,199)",	  "rgb(0,0,203)",	  "rgb(0,0,207)",
			  "rgb(0,0,211)",	  "rgb(0,0,215)",	  "rgb(0,0,219)",	  "rgb(0,0,223)",	  "rgb(0,0,227)",
			  "rgb(0,0,231)",	  "rgb(0,0,235)",	  "rgb(0,0,239)",	  "rgb(0,0,243)",	  "rgb(0,0,247)",
			  "rgb(0,0,251)",	  "rgb(0,0,255)",	  "rgb(0,4,255)",	  "rgb(0,8,255)",	  "rgb(0,12,255)",
			  "rgb(0,16,255)",	  "rgb(0,20,255)",	  "rgb(0,24,255)",	  "rgb(0,28,255)",	  "rgb(0,32,255)",
			  "rgb(0,36,255)",	  "rgb(0,40,255)",	  "rgb(0,44,255)",	  "rgb(0,48,255)",	  "rgb(0,52,255)",
			  "rgb(0,56,255)",	  "rgb(0,60,255)",	  "rgb(0,64,255)",	  "rgb(0,68,255)",	  "rgb(0,72,255)",
			  "rgb(0,76,255)",	  "rgb(0,80,255)",	  "rgb(0,84,255)",	  "rgb(0,88,255)",	  "rgb(0,92,255)",
			  "rgb(0,96,255)",	  "rgb(0,100,255)",	  "rgb(0,104,255)",	  "rgb(0,108,255)",	  "rgb(0,112,255)",
			  "rgb(0,116,255)",	  "rgb(0,120,255)",	  "rgb(0,124,255)",	  "rgb(0,128,255)",	  "rgb(0,131,255)",
			  "rgb(0,135,255)",	  "rgb(0,139,255)",	  "rgb(0,143,255)",	  "rgb(0,147,255)",	  "rgb(0,151,255)",
			  "rgb(0,155,255)",	  "rgb(0,159,255)",	  "rgb(0,163,255)",	  "rgb(0,167,255)",	  "rgb(0,171,255)",
			  "rgb(0,175,255)",	  "rgb(0,179,255)",	  "rgb(0,183,255)",	  "rgb(0,187,255)",	  "rgb(0,191,255)",
			  "rgb(0,195,255)",	  "rgb(0,199,255)",	  "rgb(0,203,255)",	  "rgb(0,207,255)",	  "rgb(0,211,255)",
			  "rgb(0,215,255)",	  "rgb(0,219,255)",	  "rgb(0,223,255)",	  "rgb(0,227,255)",	  "rgb(0,231,255)",
			  "rgb(0,235,255)",	  "rgb(0,239,255)",	  "rgb(0,243,255)",	  "rgb(0,247,255)",	  "rgb(0,251,255)",
			  "rgb(0,255,255)",	  "rgb(4,255,251)",	  "rgb(8,255,247)",	  "rgb(12,255,243)",  "rgb(16,255,239)",
			  "rgb(20,255,235)",  "rgb(24,255,231)",  "rgb(28,255,227)",  "rgb(32,255,223)",  "rgb(36,255,219)",
			  "rgb(40,255,215)",  "rgb(44,255,211)",  "rgb(48,255,207)",  "rgb(52,255,203)",  "rgb(56,255,199)",
			  "rgb(60,255,195)",  "rgb(64,255,191)",  "rgb(68,255,187)",  "rgb(72,255,183)",  "rgb(76,255,179)",
			  "rgb(80,255,175)",  "rgb(84,255,171)",  "rgb(88,255,167)",  "rgb(92,255,163)",  "rgb(96,255,159)",
			  "rgb(100,255,155)", "rgb(104,255,151)", "rgb(108,255,147)", "rgb(112,255,143)", "rgb(116,255,139)",
			  "rgb(120,255,135)", "rgb(124,255,131)", "rgb(128,255,128)", "rgb(131,255,124)", "rgb(135,255,120)",
			  "rgb(139,255,116)", "rgb(143,255,112)", "rgb(147,255,108)", "rgb(151,255,104)", "rgb(155,255,100)",
			  "rgb(159,255,96)",  "rgb(163,255,92)",  "rgb(167,255,88)",  "rgb(171,255,84)",  "rgb(175,255,80)",
			  "rgb(179,255,76)",  "rgb(183,255,72)",  "rgb(187,255,68)",  "rgb(191,255,64)",  "rgb(195,255,60)",
			  "rgb(199,255,56)",  "rgb(203,255,52)",  "rgb(207,255,48)",  "rgb(211,255,44)",  "rgb(215,255,40)",
			  "rgb(219,255,36)",  "rgb(223,255,32)",  "rgb(227,255,28)",  "rgb(231,255,24)",  "rgb(235,255,20)",
			  "rgb(239,255,16)",  "rgb(243,255,12)",  "rgb(247,255,8)",	  "rgb(251,255,4)",	  "rgb(255,255,0)",
			  "rgb(255,251,0)",	  "rgb(255,247,0)",	  "rgb(255,243,0)",	  "rgb(255,239,0)",	  "rgb(255,235,0)",
			  "rgb(255,231,0)",	  "rgb(255,227,0)",	  "rgb(255,223,0)",	  "rgb(255,219,0)",	  "rgb(255,215,0)",
			  "rgb(255,211,0)",	  "rgb(255,207,0)",	  "rgb(255,203,0)",	  "rgb(255,199,0)",	  "rgb(255,195,0)",
			  "rgb(255,191,0)",	  "rgb(255,187,0)",	  "rgb(255,183,0)",	  "rgb(255,179,0)",	  "rgb(255,175,0)",
			  "rgb(255,171,0)",	  "rgb(255,167,0)",	  "rgb(255,163,0)",	  "rgb(255,159,0)",	  "rgb(255,155,0)",
			  "rgb(255,151,0)",	  "rgb(255,147,0)",	  "rgb(255,143,0)",	  "rgb(255,139,0)",	  "rgb(255,135,0)",
			  "rgb(255,131,0)",	  "rgb(255,128,0)",	  "rgb(255,124,0)",	  "rgb(255,120,0)",	  "rgb(255,116,0)",
			  "rgb(255,112,0)",	  "rgb(255,108,0)",	  "rgb(255,104,0)",	  "rgb(255,100,0)",	  "rgb(255,96,0)",
			  "rgb(255,92,0)",	  "rgb(255,88,0)",	  "rgb(255,84,0)",	  "rgb(255,80,0)",	  "rgb(255,76,0)",
			  "rgb(255,72,0)",	  "rgb(255,68,0)",	  "rgb(255,64,0)",	  "rgb(255,60,0)",	  "rgb(255,56,0)",
			  "rgb(255,52,0)",	  "rgb(255,48,0)",	  "rgb(255,44,0)",	  "rgb(255,40,0)",	  "rgb(255,36,0)",
			  "rgb(255,32,0)",	  "rgb(255,28,0)",	  "rgb(255,24,0)",	  "rgb(255,20,0)",	  "rgb(255,16,0)",
			  "rgb(255,12,0)",	  "rgb(255,8,0)",	  "rgb(255,4,0)",	  "rgb(255,0,0)",	  "rgb(251,0,0)",
			  "rgb(247,0,0)",	  "rgb(243,0,0)",	  "rgb(239,0,0)",	  "rgb(235,0,0)",	  "rgb(231,0,0)",
			  "rgb(227,0,0)",	  "rgb(223,0,0)",	  "rgb(219,0,0)",	  "rgb(215,0,0)",	  "rgb(211,0,0)",
			  "rgb(207,0,0)",	  "rgb(203,0,0)",	  "rgb(199,0,0)",	  "rgb(195,0,0)",	  "rgb(191,0,0)",
			  "rgb(187,0,0)",	  "rgb(183,0,0)",	  "rgb(179,0,0)",	  "rgb(175,0,0)",	  "rgb(171,0,0)",
			  "rgb(167,0,0)",	  "rgb(163,0,0)",	  "rgb(159,0,0)",	  "rgb(155,0,0)",	  "rgb(151,0,0)",
			  "rgb(147,0,0)",	  "rgb(143,0,0)",	  "rgb(139,0,0)",	  "rgb(135,0,0)",	  "rgb(131,0,0)",
			  "rgb(128,0,0)" });
	}
	else if (map_name == "plasma")
	{
		colormap.assign({ "rgb(13,8,135)",	 "rgb(16,7,136)",	"rgb(19,7,137)",   "rgb(22,7,138)",	  "rgb(25,6,140)",
						  "rgb(27,6,141)",	 "rgb(29,6,142)",	"rgb(32,6,143)",   "rgb(34,6,144)",	  "rgb(36,6,145)",
						  "rgb(38,5,145)",	 "rgb(40,5,146)",	"rgb(42,5,147)",   "rgb(44,5,148)",	  "rgb(46,5,149)",
						  "rgb(47,5,150)",	 "rgb(49,5,151)",	"rgb(51,5,151)",   "rgb(53,4,152)",	  "rgb(55,4,153)",
						  "rgb(56,4,154)",	 "rgb(58,4,154)",	"rgb(60,4,155)",   "rgb(62,4,156)",	  "rgb(63,4,156)",
						  "rgb(65,4,157)",	 "rgb(67,3,158)",	"rgb(68,3,158)",   "rgb(70,3,159)",	  "rgb(72,3,159)",
						  "rgb(73,3,160)",	 "rgb(75,3,161)",	"rgb(76,2,161)",   "rgb(78,2,162)",	  "rgb(80,2,162)",
						  "rgb(81,2,163)",	 "rgb(83,2,163)",	"rgb(85,2,164)",   "rgb(86,1,164)",	  "rgb(88,1,164)",
						  "rgb(89,1,165)",	 "rgb(91,1,165)",	"rgb(92,1,166)",   "rgb(94,1,166)",	  "rgb(96,1,166)",
						  "rgb(97,0,167)",	 "rgb(99,0,167)",	"rgb(100,0,167)",  "rgb(102,0,167)",  "rgb(103,0,168)",
						  "rgb(105,0,168)",	 "rgb(106,0,168)",	"rgb(108,0,168)",  "rgb(110,0,168)",  "rgb(111,0,168)",
						  "rgb(113,0,168)",	 "rgb(114,1,168)",	"rgb(116,1,168)",  "rgb(117,1,168)",  "rgb(119,1,168)",
						  "rgb(120,1,168)",	 "rgb(122,2,168)",	"rgb(123,2,168)",  "rgb(125,3,168)",  "rgb(126,3,168)",
						  "rgb(128,4,168)",	 "rgb(129,4,167)",	"rgb(131,5,167)",  "rgb(132,5,167)",  "rgb(134,6,166)",
						  "rgb(135,7,166)",	 "rgb(136,8,166)",	"rgb(138,9,165)",  "rgb(139,10,165)", "rgb(141,11,165)",
						  "rgb(142,12,164)", "rgb(143,13,164)", "rgb(145,14,163)", "rgb(146,15,163)", "rgb(148,16,162)",
						  "rgb(149,17,161)", "rgb(150,19,161)", "rgb(152,20,160)", "rgb(153,21,159)", "rgb(154,22,159)",
						  "rgb(156,23,158)", "rgb(157,24,157)", "rgb(158,25,157)", "rgb(160,26,156)", "rgb(161,27,155)",
						  "rgb(162,29,154)", "rgb(163,30,154)", "rgb(165,31,153)", "rgb(166,32,152)", "rgb(167,33,151)",
						  "rgb(168,34,150)", "rgb(170,35,149)", "rgb(171,36,148)", "rgb(172,38,148)", "rgb(173,39,147)",
						  "rgb(174,40,146)", "rgb(176,41,145)", "rgb(177,42,144)", "rgb(178,43,143)", "rgb(179,44,142)",
						  "rgb(180,46,141)", "rgb(181,47,140)", "rgb(182,48,139)", "rgb(183,49,138)", "rgb(184,50,137)",
						  "rgb(186,51,136)", "rgb(187,52,136)", "rgb(188,53,135)", "rgb(189,55,134)", "rgb(190,56,133)",
						  "rgb(191,57,132)", "rgb(192,58,131)", "rgb(193,59,130)", "rgb(194,60,129)", "rgb(195,61,128)",
						  "rgb(196,62,127)", "rgb(197,64,126)", "rgb(198,65,125)", "rgb(199,66,124)", "rgb(200,67,123)",
						  "rgb(201,68,122)", "rgb(202,69,122)", "rgb(203,70,121)", "rgb(204,71,120)", "rgb(204,73,119)",
						  "rgb(205,74,118)", "rgb(206,75,117)", "rgb(207,76,116)", "rgb(208,77,115)", "rgb(209,78,114)",
						  "rgb(210,79,113)", "rgb(211,81,113)", "rgb(212,82,112)", "rgb(213,83,111)", "rgb(213,84,110)",
						  "rgb(214,85,109)", "rgb(215,86,108)", "rgb(216,87,107)", "rgb(217,88,106)", "rgb(218,90,106)",
						  "rgb(218,91,105)", "rgb(219,92,104)", "rgb(220,93,103)", "rgb(221,94,102)", "rgb(222,95,101)",
						  "rgb(222,97,100)", "rgb(223,98,99)",	"rgb(224,99,99)",  "rgb(225,100,98)", "rgb(226,101,97)",
						  "rgb(226,102,96)", "rgb(227,104,95)", "rgb(228,105,94)", "rgb(229,106,93)", "rgb(229,107,93)",
						  "rgb(230,108,92)", "rgb(231,110,91)", "rgb(231,111,90)", "rgb(232,112,89)", "rgb(233,113,88)",
						  "rgb(233,114,87)", "rgb(234,116,87)", "rgb(235,117,86)", "rgb(235,118,85)", "rgb(236,119,84)",
						  "rgb(237,121,83)", "rgb(237,122,82)", "rgb(238,123,81)", "rgb(239,124,81)", "rgb(239,126,80)",
						  "rgb(240,127,79)", "rgb(240,128,78)", "rgb(241,129,77)", "rgb(241,131,76)", "rgb(242,132,75)",
						  "rgb(243,133,75)", "rgb(243,135,74)", "rgb(244,136,73)", "rgb(244,137,72)", "rgb(245,139,71)",
						  "rgb(245,140,70)", "rgb(246,141,69)", "rgb(246,143,68)", "rgb(247,144,68)", "rgb(247,145,67)",
						  "rgb(247,147,66)", "rgb(248,148,65)", "rgb(248,149,64)", "rgb(249,151,63)", "rgb(249,152,62)",
						  "rgb(249,154,62)", "rgb(250,155,61)", "rgb(250,156,60)", "rgb(250,158,59)", "rgb(251,159,58)",
						  "rgb(251,161,57)", "rgb(251,162,56)", "rgb(252,163,56)", "rgb(252,165,55)", "rgb(252,166,54)",
						  "rgb(252,168,53)", "rgb(252,169,52)", "rgb(253,171,51)", "rgb(253,172,51)", "rgb(253,174,50)",
						  "rgb(253,175,49)", "rgb(253,177,48)", "rgb(253,178,47)", "rgb(253,180,47)", "rgb(253,181,46)",
						  "rgb(254,183,45)", "rgb(254,184,44)", "rgb(254,186,44)", "rgb(254,187,43)", "rgb(254,189,42)",
						  "rgb(254,190,42)", "rgb(254,192,41)", "rgb(253,194,41)", "rgb(253,195,40)", "rgb(253,197,39)",
						  "rgb(253,198,39)", "rgb(253,200,39)", "rgb(253,202,38)", "rgb(253,203,38)", "rgb(252,205,37)",
						  "rgb(252,206,37)", "rgb(252,208,37)", "rgb(252,210,37)", "rgb(251,211,36)", "rgb(251,213,36)",
						  "rgb(251,215,36)", "rgb(250,216,36)", "rgb(250,218,36)", "rgb(249,220,36)", "rgb(249,221,37)",
						  "rgb(248,223,37)", "rgb(248,225,37)", "rgb(247,226,37)", "rgb(247,228,37)", "rgb(246,230,38)",
						  "rgb(246,232,38)", "rgb(245,233,38)", "rgb(245,235,39)", "rgb(244,237,39)", "rgb(243,238,39)",
						  "rgb(243,240,39)", "rgb(242,242,39)", "rgb(241,244,38)", "rgb(241,245,37)", "rgb(240,247,36)",
						  "rgb(240,249,33)" });
	}
	else
	{
		std::cout << "WARNING : The colormap " << map_name << " has not been specified for the SVG substrate plot."
				  << std::endl
				  << "\tUsing YlOrRd." << std::endl
				  << std::endl;
		settings.svg_substrate_colormap = "YlOrRd";
		setup_svg_substrate_colormap(colormap, settings);
	}
