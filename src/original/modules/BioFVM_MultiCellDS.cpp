...
{
	physicell::RUNTIME_TIC();
	physicell::RUNTIME_TOC();
	current_runtime = physicell::runtime_stopwatch_value();
	return;
}
...
void MultiCellDS_Metadata::add_to_open_xml_pugi(double current_simulation_time, pugi::xml_document& xml_dom)
{
	...
	physicell::RUNTIME_TOC();
	BioFVM_metadata.current_runtime = physicell::runtime_stopwatch_value();
...
void set_save_biofvm_mesh_as_matlab(bool newvalue) { save_mesh_as_matlab = newvalue; }

void set_save_biofvm_data_as_matlab(bool newvalue) { save_density_data_as_matlab = newvalue; }

void set_save_biofvm_cell_data(bool newvalue) { save_cell_data = newvalue; }

void set_save_biofvm_cell_data_as_custom_matlab(bool newvalue) { save_cells_as_custom_matlab = newvalue; }

/* writing parts of BioFVM to a MultiCellDS file */

void write_microenvironment_to_matlab(std::string filename, microenvironment& m)
{
	auto number_of_data_entries = m.mesh.voxel_count();
	int size_of_each_datum = 3 + 1 + m.substrates_count;
...
void add_BioFVM_substrates_to_open_xml_pugi(pugi::xml_document& xml_dom, std::string filename_base, microenvironment& M)
{
	...
	if (!BioFVM_substrates_initialized_in_dom)
	{
		...
		attrib.set_value(true);
		attrib = node.append_attribute("regular");
		attrib.set_value(true);
		attrib = node.append_attribute("units");
		attrib.set_value(M.space_units.c_str());
		...
		attrib = node.append_attribute("units");
		attrib.set_value(M.space_units.c_str());
		sprintf(buffer, "%f %f %f %f %f %f", (double)M.mesh.bounding_box_mins[0], (double)M.mesh.bounding_box_mins[1],
				(double)M.mesh.bounding_box_mins[2], (double)M.mesh.bounding_box_maxs[0],
				(double)M.mesh.bounding_box_maxs[1], (double)M.mesh.bounding_box_maxs[2]);
		node.append_child(pugi::node_pcdata).set_value(buffer);
		node = node.parent();
		...
		char temp[1024];
		for (auto j = 0; j < M.substrates_count; j++)
		{
			node = node.append_child("variable");
			attrib = node.append_attribute("name");
			attrib.set_value(M.substrates_names[j].c_str());
			attrib = node.append_attribute("units");
			attrib.set_value(M.substrates_names[j].c_str());
			attrib = node.append_attribute("ID");
			attrib.set_value(j);
			...
			node.append_child(pugi::node_pcdata).set_value(temp);
			attrib = node.append_attribute("units");
			sprintf(temp, "%s^2/%s", M.space_units.c_str(), M.time_units.c_str());
		...
		if (save_density_data_as_matlab == false)
		{
			...
			int datum_size = 16; // enough for sprintf default 6 decimal places + period + 5 leading figs (safety) +
								 // delimiter + 2 chars safety = 15
			int data_size = datum_size * M.substrates_count;

			char* buffer;
			buffer = new char[data_size];
			for (unsigned int j = 0; j < M.mesh.voxel_count(); j++)
			{
				physicell::data_to_list(M.substrate_densities.get() + M.substrates_count * j, M.substrates_count,
										buffer, ' ');
		...
		else
		{
			...
			write_microenvironment_to_matlab(filename, M);
	...
	if (save_density_data_as_matlab == false)
	{
		...
		for (unsigned int j = 0; j < M.mesh.voxel_count(); j++)
		{
			physicell::data_to_list(M.substrate_densities.get() + M.substrates_count * j, M.substrates_count, buffer,
									' ');
...
void add_BioFVM_agents_to_open_xml_pugi(pugi::xml_document& xml_dom, std::string filename_base, microenvironment& M)
{
	...
	if (!initialized)
	{
		...
		sprintf(volume_chars, "%s^3", M.space_units.c_str());
		sprintf(diffusion_chars, "%s^2/%s", M.space_units.c_str(), M.time_units.c_str());
	}
	...
	if (save_cells_as_custom_matlab == true)
	{
		...
		int number_of_data_entries = M.agents->agents_count();
		int size_of_each_datum =
			1 + 3 + 1 + 3 * M.substrates_count; // ID, x,y,z, volume,  src,sink,saturation (multiple)

		FILE* fp = physicell::write_matlab_header(size_of_each_datum, number_of_data_entries, filename, "basic_agents");

		// storing data as cols
		for (int i = 0; i < number_of_data_entries; i++)
		{
			double ID_temp = (double)M.agents->get_agent_at(i)->id;
			fwrite((char*)&(ID_temp), sizeof(double), 1, fp);

			auto pos = M.agents->get_agent_at(i)->get_position();
			point_t<double, 3> tmppos = { pos[0], pos[1], pos[2] };

			fwrite((char*)tmppos.data(), sizeof(double), 3, fp);
			double volTemp = M.agents->get_agent_at(i)->volume();
			fwrite((char*)&(volTemp), sizeof(double), 1, fp);

			// add variables and their source/sink/saturation values (per-cell basis)
			for (auto j = 0; j < M.substrates_count; j++)
			{
				double dTemp = M.agents->get_agent_at(i)->volume() * M.agents->get_agent_at(i)->secretion_rates()[j];
				fwrite((char*)&(dTemp), sizeof(double), 1, fp);
				dTemp = M.agents->get_agent_at(i)->volume() * M.agents->get_agent_at(i)->uptake_rates()[j];
				fwrite((char*)&(dTemp), sizeof(double), 1, fp);
				dTemp = M.agents->get_agent_at(i)->saturation_densities()[j];
				fwrite((char*)&(dTemp), sizeof(double), 1, fp);
			}
		}
	...
	root = node;
	for (auto i = 0; i < M.agents->agents_count(); i++)
	{
		node = node.append_child("cell");
		attrib = node.append_attribute("ID");
		attrib.set_value(M.agents->get_agent_at(i)->id);
		...
		for (auto j = 0; j < M.substrates_count; j++)
		{
			...
			sprintf(temp, "%f", M.agents->get_agent_at(i)->volume() * M.agents->get_agent_at(i)->secretion_rates()[j]);
			node.append_child(pugi::node_pcdata).set_value(temp);
			node = node.parent();
			...
			attrib = node.append_attribute("units");
			attrib.set_value(rate_chars);
			sprintf(temp, "%f", M.agents->get_agent_at(i)->volume() * M.agents->get_agent_at(i)->uptake_rates()[j]);
			node.append_child(pugi::node_pcdata).set_value(temp);
			node = node.parent();

			node = node.append_child("saturation_density");
			attrib = node.append_attribute("units");
			attrib.set_value(M.substrates_units[j].c_str());
			sprintf(temp, "%f", M.agents->get_agent_at(i)->saturation_densities()[j]);
			node.append_child(pugi::node_pcdata).set_value(temp);
			node = node.parent();
		...
		sprintf(temp, "%f", M.agents->get_agent_at(i)->volume());
		node.append_child(pugi::node_pcdata).set_value(temp);
		node = node.parent();
		node = node.parent();
		...
		// vector3_to_list( M.agents->get_agent_at(i)->position , temp , ' ');
		sprintf(temp, "%.7e %.7e %.7e", M.agents->get_agent_at(i)->get_position()[0],
				M.agents->get_agent_at(i)->get_position()[1], M.agents->get_agent_at(i)->get_position()[2]);
		node.append_child(pugi::node_pcdata).set_value(temp);
...
// 		if (strcmp(node.attribute("type").value(), "Cartesian") != 0
// 			&& strcmp(node.attribute("type").value(), "cartesian") != 0)
// 		{
// 			cartesian = false;
// 		}
...
// 			{
// 				std::cout
// 					<< "Warning! BioFVM / MultiCellDS cannot currently fully initialize to non-regular Cartesian data!"
// 					<< std::endl;
// 			}
// 		}
...
// 	{
// 		// std::cout << microenvironment.mesh.voxels[n].center << " vs " << mat[0][n] << " " << mat[1][n] << " " <<
// 		// mat[2][n] << std::endl;
