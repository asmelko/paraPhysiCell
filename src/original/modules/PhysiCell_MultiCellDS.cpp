...
void add_PhysiCell_cells_to_open_xml_pugi(pugi::xml_document& xml_dom, std::string filename_base, environment& e)
{
	...
	if (save_cells_as_custom_matlab == true || 1 == 1)
	{
		...
		// cell interactions:
		size_of_each_datum += 1 + number_of_cell_defs + number_of_cell_defs + 1 + number_of_cell_defs;
		...
		{
			std::cout << std::endl
					  << "Error: Failed to open " << filename << " for MAT writing." << std::endl
					  << std::endl;
		...
		for (int i = 0; i < number_of_data_entries; i++)
		{
			...
			write_to_file(pCell->phenotype.cycle.data.elapsed_time_in_phase,
						  fp); // elapsed time in phase

			// volume information
			// nuclear volume, cytoplasmic volume, fluid fraction, calcified fraction,
			write_to_file(pCell->phenotype.volume.nuclear(), fp);			 // nuclear volume
			write_to_file(pCell->phenotype.volume.cytoplasmic(), fp);		 // cytoplasmic volume
			write_to_file(pCell->phenotype.volume.fluid_fraction(), fp);	 // fluid fraction
			write_to_file(pCell->phenotype.volume.calcified_fraction(), fp); // calcified fraction
			...
			write_to_file(pCell->phenotype.motility.migration_speed(), fp);								   // speed
			write_to_file(fill(pCell->phenotype.motility.motility_vector(), e.m.mesh.dims).data(), 3, fp); // velocity
			write_to_file(pCell->phenotype.motility.migration_bias(), fp); // bias (0 to 1)
			write_to_file(fill(pCell->phenotype.motility.migration_bias_direction(), e.m.mesh.dims).data(), 3,
						  fp);												 // bias direction
			write_to_file(pCell->phenotype.motility.persistence_time(), fp); // persistence
			fwrite((char*)&(temp_zero), sizeof(double), 1, fp);				 // reserved for "time in this direction"


			// new in 2022: interactions :
			// fwrite( (char*) &( pCell->phenotype.motility.chemotactic_sensitivities ) , sizeof(double) ,
			// number_of_cell_defs , fp ); // chemotactic_sensitivities
			write_to_file(pCell->phenotype.motility.chemotactic_sensitivities(), number_of_substrates,
						  fp); // chemotactic_sensitivities
			write_to_file(pCell->phenotype.mechanics.cell_adhesion_affinities(), number_of_cell_defs,
						  fp); // cell_adhesion_affinities
			write_to_file(pCell->phenotype.cell_interactions.dead_phagocytosis_rate(),
						  fp); // dead_phagocytosis_rate
			write_to_file(pCell->phenotype.cell_interactions.live_phagocytosis_rates(), number_of_cell_defs,
						  fp); // live_phagocytosis_rates
			write_to_file(pCell->phenotype.cell_interactions.attack_rates(), number_of_cell_defs,
						  fp); // attack_rates
			write_to_file(pCell->phenotype.cell_interactions.damage_rate(),
						  fp); // damage_rate
			write_to_file(pCell->phenotype.cell_interactions.fusion_rates(), number_of_cell_defs,
						  fp); // fusion_rates
			write_to_file(pCell->phenotype.cell_transformations.transformation_rates(), number_of_cell_defs,
						  fp); // transformation_rates
	...
	for (int i = 0; i < e.cast_container<cell_container>().agents_count(); i++)
	{
		...
		for (int j = 0; j < e.m.substrates_count; j++)
		{
			node = node.append_child("variable");
			attrib = node.append_attribute("name");
			attrib.set_value(e.m.substrates_names[j].c_str());
...
void add_PhysiCell_cells_to_open_xml_pugi_v2(pugi::xml_document& xml_dom, std::string filename_base, environment& e)
{
	...
	for (int i = 0; i < number_of_data_entries; i++)
	{
		...
		// interactions
		// name = "dead_phagocytosis_rate";
		write_to_file(pCell->phenotype.cell_interactions.dead_phagocytosis_rate(), fp);
		// name = "live_phagocytosis_rates";
		...
		// name = "damage_rate";
		write_to_file(pCell->phenotype.cell_interactions.damage_rate(), fp);
		// name = "fusion_rates";
